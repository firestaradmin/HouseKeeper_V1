...
Description:将int型转换为unsigned char*
Input:		int n ：要转换的数
Return:		unsigned char* :转换后的字符串指针
...
//--------------------------------------------------------------
// Prototype      : void OLED_ShowIntNum(unsigned char x, unsigned char y, unsigned char ch[], unsigned char TextSize)
// Calls          : 
// Parameters     : x,y -- 起始点坐标(x:0~127, y:0~7); int num 显示的int整型; TextSize -- 字符大小(1:6*8 ; 2:8*16)
// Description    : 显示codetab.h中的ASCII字符,有6*8和8*16可选择
//--------------------------------------------------------------
void OLED_ShowIntNum(unsigned char x, unsigned char y, int num, unsigned char TextSize)
{
	unsigned char c = 0,i = 0,j = 0;

...
* History		:
2020.01.06		：增加了SPI模式，将ssd1306的IIC和SPI基本整合到一起，方便调用
...
u8 OLED_GRAM[8][128];			//定义模拟显存 1024 Bytes
...
void OLED_Init(void)
{
	...
	//WriteCmd(0x78); //--set start line address		//一般都是设为0x40   但是有些屏幕可能有问题，需要重新映射第0行
	...
	OLED_RamClear();//清空显存ram
		...
		#ifdef OLED_I2C_Mode
			MYDMA_Config(DMA1_Channel6,(u32)&OLED_HardWare_I2Cx->DR,(u32)OLED_GRAM,1025);//DMA1通道4,外设为I2C1,存储器为OLED_GRAM,长度128*8 = 1024.
			I2C_DMACmd(OLED_HardWare_I2Cx, ENABLE);//使能I2C 的 DMA请求
		#else //OLED_SPI_Mode
			MYDMA_Config(DMA1_Channel3,(u32)&OLED_HardWare_SPIx->DR,(u32)OLED_GRAM,1024);//DMA1通道4,外设为I2C1,存储器为OLED_GRAM,长度128*8 = 1024.
			SPI_I2S_DMACmd(OLED_HardWare_SPIx, SPI_I2S_DMAReq_Tx, ENABLE);//使能SPI 的 DMA请求
		#endif
	#endif
...
void OLED_Refresh_OneTime(void)
{
				...
				if(g_OLED_DMA_BusyFlag == 0)
				{
					...
					I2C_GenerateSTART(OLED_HardWare_I2Cx, ENABLE);//开启I2C1
					while(!I2C_CheckEvent(OLED_HardWare_I2Cx, I2C_EVENT_MASTER_MODE_SELECT));/*EV5,主模式*/

					I2C_Send7bitAddress(OLED_HardWare_I2Cx, OLED_ADDRESS, I2C_Direction_Transmitter);//器件地址 -- 默认0x78
					...
					I2C_SendData(OLED_HardWare_I2Cx, OLED_DATA);//寄存器地址
				...
				for(i = 0; i < 8;  i++)
				{
					for(j = 0;j<128;j++)
					{
						I2C_SendData(OLED_HardWare_I2Cx, OLED_GRAM[i][j]);//发送数据
						while (!I2C_CheckEvent(OLED_HardWare_I2Cx, I2C_EVENT_MASTER_BYTE_TRANSMITTED));
					}
					
				}
				
				I2C_GenerateSTOP(OLED_HardWare_I2Cx, ENABLE);//关闭I2C1总线
...
void MYDMA_Config(DMA_Channel_TypeDef* DMA_CHx,u32 cpar,u32 cmar,u16 cndtr)
{
	...
	#else
		NVIC_InitStructure.NVIC_IRQChannel = DMA1_Channel3_IRQn;  //DMA中断
	#endif
...
//{
//	MYDMA_Config(DMA1_Channel6,(u32)&OLED_HardWare_I2Cx->DR,(u32)OLED_GRAM,1025);//DMA1通道4,外设为I2C1,存储器为OLED_GRAM,长度128*8 = 1024.
//	I2C_DMACmd(OLED_HardWare_I2Cx, ENABLE);//使能I2C1 的 DMA请求
//	
//}
...
void OLED_I2C_Configuration(void)
{
	...
	I2C_DeInit(OLED_HardWare_I2Cx);//使用I2C1
...
void OLED_SPI_Configuration(void)
{
		...
		GPIO_SetBits(OLED_RST_PORT,OLED_RST_PIN);					//初始高电平
		/* CS的IO口设置 */
		GPIO_InitStructure.GPIO_Pin = OLED_CS_PIN;  
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;  		//推挽输出
		GPIO_InitStructure.GPIO_Speed=GPIO_Speed_50MHz;
		GPIO_Init(OLED_CS_PORT, &GPIO_InitStructure);
		GPIO_SetBits(OLED_CS_PORT,OLED_CS_PIN);					//初始高电平
		/* DC的IO口设置 */
		GPIO_InitStructure.GPIO_Pin = OLED_DC_PIN;  
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;  		//推挽输出
		GPIO_InitStructure.GPIO_Speed=GPIO_Speed_50MHz;
		GPIO_Init(OLED_DC_PORT, &GPIO_InitStructure);
		GPIO_SetBits(OLED_DC_PORT,OLED_DC_PIN);					//初始高电平
		...
		SPI_InitStructure.SPI_BaudRatePrescaler = OLED_SPI_BaudRatePrescaler;// 定义波特率预分频的值:波特率预分频值为16
		SPI_InitStructure.SPI_FirstBit = SPI_FirstBit_MSB;// 指定数据传输从MSB位还是LSB位开始:数据传输从MSB位开始
		SPI_InitStructure.SPI_CRCPolynomial = 7;		  // CRC值计算的多项式
		SPI_Init(OLED_HardWare_SPIx, &SPI_InitStructure);  			  // 根据SPI_InitStruct中指定的参数初始化外设SPIx寄存器
		
		SPI_Cmd(OLED_HardWare_SPIx, ENABLE); 	  // 使能SPI外设
	#else 
...
void OLED_WriteByte(uint8_t addr,uint8_t data)
{
			...
			I2C_SendData(OLED_HardWare_I2Cx, addr);//寄存器地址
			while (!I2C_CheckEvent(OLED_HardWare_I2Cx, I2C_EVENT_MASTER_BYTE_TRANSMITTED));

			I2C_SendData(OLED_HardWare_I2Cx, data);//发送数据
			while (!I2C_CheckEvent(OLED_HardWare_I2Cx, I2C_EVENT_MASTER_BYTE_TRANSMITTED));
...
void SPI_SendData(SPI_TypeDef* SPIx,u8 byte)
{
	...
	SPI_I2S_SendData(SPIx, byte);									   	// 发送一个字节  
	while (SPI_I2S_GetFlagStatus(SPIx, SPI_I2S_FLAG_TXE) == RESET);  	// 等待发送区空

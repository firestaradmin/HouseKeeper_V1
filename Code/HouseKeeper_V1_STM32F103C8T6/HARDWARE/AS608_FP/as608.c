	...
	while(1)
	{
		switch (processnum)
		{
			...
			case 3:
				...
				if(ensure==0x00) 
				{
					printf("enerate fingerprint success!\r\n");
					processnum=4;//跳到第五步
			...
			case 4:		
				...
				{			
					...
					printf("剩余指纹个数:%d", AS608Para.PS_max-ValidN);
...
u16 GET_NUM(void)
{
	...
	num = (USART_RX_BUF[0] - '0') * 100 + (USART_RX_BUF[1] - '0') * 10 + (USART_RX_BUF[2] - '0') ;
	return num;
...
void Del_FR(void)
{
	...
	{	
		...
		ShowErrMessage(ensure);	
	PS_ValidTempleteNum(&ValidN);//读库指纹个数
	printf("剩余指纹个数:%d", AS608Para.PS_max-ValidN);//显示剩余指纹个数
 MENU:
	return;
}
